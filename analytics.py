...
def load_data(file):
    ...
    for key in PERIODS.values():
        ...
        if rcol in df.columns:
            df[rcol] = df[rcol].astype(str).str.strip().str.capitalize().replace("Nan", "")
...
with c1:
    st.subheader("📉 Rating Trend Across Periods")
    trend_rows = []
    ...
    if trend_rows:
        fig_trend = px.bar(
            pd.DataFrame(trend_rows), x="Period", y="Teachers", color="Rating",
            color_discrete_map=RATING_COLORS, barmode="stack",
            category_orders={"Rating": ["Red", "Amber", "Yellow", "Green"]},
        )
        fig_trend.update_layout(xaxis_title="", legend_title="Rating")
